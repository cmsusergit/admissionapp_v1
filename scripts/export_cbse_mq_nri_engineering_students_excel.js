...
const outputExcelPath = path.join(process.cwd(), 'cbse_mq_nri_engineering_students_with_percentile.xlsx');
...
/**
 * Loads standard PCM -> PERCENTILE lookup table (cbsepdf.xlsx)
 */
function loadStandardPercentileLookup(filePath) {
  console.log(`📄 Reading CBSE lookup file: ${filePath}`);
...
async function exportCbseStudents() {
  console.log('🚀 Starting CBSE Student Export with Board & GUJCET Percentiles...');
  ...
  for (const app of applications || []) {
    ...
    const isCbse = boardStr.toUpperCase().includes('CBSE') || boardStr.toUpperCase().includes('CENTRAL BOARD');
    if (!isCbse) continue;
    ...
    if (physicsTheory !== null && chemistryTheory !== null && mathTheory !== null) {
      ...
      boardPercentile = lookupPercentile(boardPcmTotal, cbseLookupMap);
    }
  ...
  console.log(`✅ Extracted ${excelRows.length} CBSE student records.\n`);

  if (excelRows.length > 0) {
    const worksheet = XLSX.utils.json_to_sheet(excelRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'CBSE_Students_Percentile');
    ...
    console.log(`🎉 CBSE Excel file generated successfully!`);
