...
async function backfillDocuments() {
    console.log('--- Starting Deep Document Backfill ---');
    ...
    for (const app of applications) {
        ...
        for (const [key, value] of dataEntries) {
            // Check if the value looks like a document path
            // e.g. "student_id/filename.ext" or contains a UUID and a extension
            if (typeof value === 'string' && value.includes('/') && (value.endsWith('.pdf') || value.endsWith('.jpg') || value.endsWith('.png') || value.endsWith('.jpeg'))) {
                
                console.log(`Found potential document: App=${app.id}, Key=${key}, Path=${value}`);

                // Infer document type from key (human readable)
                ...
                // Check if document record already exists by path or app+type
