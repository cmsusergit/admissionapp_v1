...
export const load = async ({ locals: { supabase, getAuthenticatedUser, userProfile }, parent }: Parameters<PageServerLoad>[0]) => {
...
export const actions = {
    updateProfile: async ({ request, locals: { supabase, getAuthenticatedUser, userProfile } }: import('./$types').RequestEvent) => {
        const authenticatedUser = await getAuthenticatedUser();
        if (!authenticatedUser || userProfile?.role !== 'student') {
            throw redirect(303, '/login');
        }
