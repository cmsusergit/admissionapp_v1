...
export const actions = {
    ...
    createAcademicYear: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
    ...
    updateAcademicYear: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || userProfile?.role !== 'admin') {
            throw redirect(303, '/login');
        }
    ...
    deleteAcademicYear: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || userProfile?.role !== 'admin') {
            throw redirect(303, '/login');
        }
    ...
    createAdmissionCycle: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || userProfile?.role !== 'admin') {
            throw redirect(303, '/login');
        }
    ...
    updateAdmissionCycle: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || userProfile?.role !== 'admin') {
            throw redirect(303, '/login');
        }
    ...
    deleteAdmissionCycle: async ({ request, locals: { supabase, getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || userProfile?.role !== 'admin') {
            throw redirect(303, '/login');
        }
