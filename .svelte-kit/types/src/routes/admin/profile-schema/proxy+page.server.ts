...
const fieldSchema = z.object({
    key: z.string().min(1).regex(/^[a-z0-9_]+$/, 'Key must be lowercase with underscores only'),
...
export const actions = {
    create: async ({ request, locals: { supabase, getAuthenticatedUser, userProfile } }: import('./$types').RequestEvent) => {
        ...
        if (!parsed.success) {
            return fail(400, { error: true, message: parsed.error.errors.map(e => e.message).join(', ') });
        }

        const { key, label, type, is_required, options } = parsed.data;
        
        let optionsJson = null;
        if (type === 'select' && options) {
            optionsJson = options.split('\n').map(o => o.trim()).filter(Boolean);
        }

        const { error } = await supabase
            .from('student_profile_fields')
            .insert({ key, label, type, is_required, options: optionsJson });

        if (error) {
            return fail(500, { error: true, message: 'Failed to create field. Key might be duplicate.' });
        }
    ...
    update: async ({ request, locals: { supabase, getAuthenticatedUser, userProfile } }: import('./$types').RequestEvent) => {
        ...
        const { error } = await supabase
            .from('student_profile_fields')
            .update({ key, label, type, is_required, options: optionsJson })
            .eq('id', id);

        if (error) {
            return fail(500, { error: true, message: 'Failed to update field. Key might be duplicate.' });
        }
