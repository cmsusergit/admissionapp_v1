...
export const actions = {
    save: async ({ request, locals: { supabase, getAuthenticatedUser, userProfile } }: import('./$types').RequestEvent) => {
    ...
    delete: async ({ request, locals: { supabase } }: import('./$types').RequestEvent) => {
        const formData = await request.formData();
        const id = formData.get('id') as string;
    ...
    preview: async ({ request, locals: { supabase, userProfile } }: import('./$types').RequestEvent) => {
        const formData = await request.formData();
        const base_table = formData.get('base_table') as string;
        const configStr = formData.get('configuration') as string;
    ...
    suggest_options: async ({ request, locals: { supabase, userProfile } }: import('./$types').RequestEvent) => {
        const formData = await request.formData();
