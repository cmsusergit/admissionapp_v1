...
export const actions = {
    create: async ({ request, locals: { getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
    ...
    delete: async ({ request, locals: { getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || (userProfile?.role !== 'univ_auth' && userProfile?.role !== 'university_auth')) return fail(401, { message: 'Unauthorized' });
    ...
    toggleStatus: async ({ request, locals: { getSession, userProfile } }: import('./$types').RequestEvent) => {
        const session = await getSession();
        if (!session || (userProfile?.role !== 'univ_auth' && userProfile?.role !== 'university_auth')) return fail(401, { message: 'Unauthorized' });
