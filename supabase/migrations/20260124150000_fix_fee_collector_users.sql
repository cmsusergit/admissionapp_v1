...
-- 3. Allow Fee Collectors to view Account Admissions (Already exists, but double checking logic)
-- Existing: CREATE POLICY "Account Admissions: Fee Collector view and update" ...
-- The existing one covers UPDATE as well, which is good. We leave it.

-- 4. Allow Fee Collectors to view Courses and Admission Cycles (Usually covered by 'authenticated')
-- "Courses: All authenticated can read" -> OK
-- "Admission Cycles: All authenticated can read" -> OK
