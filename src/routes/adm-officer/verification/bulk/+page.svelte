<script lang="ts">
    ...
    let filterStatus = data.statusFilter;
...
<div class="container-fluid mt-4 pb-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>Bulk Verification <span class="badge bg-secondary">{data.count}</span></h1>
                        ...
                        <div class="card-footer bg-white d-flex justify-content-between">
                            {#if app.status !== 'verified'}
                                <form method="POST" action="?/verifyStudent" use:enhance={() => {
                                    return async ({ result }) => {
                                        if (result.type === 'success') {
                                            toastStore.success('Verified successfully');
                                            await invalidateAll();
                                        }
                                    };
                                }}>
                                    ...
                                    <button class="btn btn-success btn-sm w-100" disabled={app.status === 'needs_correction'}>
                                        <i class="bi bi-check-all me-1"></i> Verify All & Approve
                                    </button>
                                </form>
                            {:else}
                            ...
                            <a href="/adm-officer/applications/{app.id}" class="btn btn-outline-secondary btn-sm ms-2">Details</a>
                        </div>
                    </div>
                </div>
            {/each}
