...
export const load: PageServerLoad = async ({ locals: { supabase, getAuthenticatedUser, userProfile }, url }) => {
    ...
    if (applicationId) {
        ...
        if (docsError) console.error('[DocumentsLoad] Docs Fetch Error:', docsError.message);
        documents = docs || [];
        ...
        if (appFetchError) {
            ...
            if (appData.form_data) {
                ...
                if (formSchema?.schema_json?.fields) {
    ...
    if (branchIds.length > 0) {
        const { data: branchesData } = await supabaseAdmin.from('branches').select('id, name').in('id', branchIds);
